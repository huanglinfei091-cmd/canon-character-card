...
function buildMotifDialogueBank(evidence, original) {
  ...
  const templates = [
    ["Lv.0-1 初识", motif => `『关于「${motif}」的事，${firstPerson}还不打算对刚认识的人说得太深。先从眼前开始吧。』`],
    ["Lv.2 眼熟", motif => `『${addressee}居然还记得「${motif}」……嗯，比${firstPerson}以为的更细心。』`],
    ["Lv.3 信任", motif => `『如果要说起「${motif}」，${firstPerson}想让${addressee}听见完整的来由，而不只是一个轻松的结尾。』`],
    ["Lv.4 依赖", motif => `『每次想到「${motif}」，${firstPerson}都会先确认${addressee}是不是还在。大概已经没办法装作毫不在意了。』`],
    ["Lv.5 全部", motif => `『以后无论「${motif}」把我们带到哪里，${firstPerson}都会把${addressee}写进自己的选择里——不是责任，是心愿。』`]
  ];
...
export function buildRoleplayPromptObject(card) {
  ...
  return {
    ...
    "meta": {
      ...
      "好感度显示": "格式为【好感度 ±N，当前 Lv.X（累计点数）】；无有效变动时可显示【好感度无变动】。"
    },
    "system_instruction": {
    ...
    "character_profile": {
      ...
      "好感度变动规则": {
        "点数机制": `初始 ${affinity.initialScore ?? 0} 点。${(affinity.stages || []).map(stage => `${stage.level} ${stage.name}：${stage.range}`).join("；")}。分数最低为0。`,
        "核心原则": "好感度增长依赖持续、与角色价值观一致的行动。普通寒暄只形成记录，不应快速升级；违背承诺、伤害重要之人和强迫亲密会造成更明显下降。",
    ...
    "roleplay_engine_v3": {
      "状态变量": ["当前好感度点数", "当前层级", "最近三轮有效事件", "尚未修复的冲突", "当前场景", "双方距离和关系边界"],
      "每轮执行顺序": ["识别用户行为和场景", "查找原作证据、角色母题与相近场景", "判断角色表面反应", "生成更私密但不越权的内心独白", "计算好感度变化", "检查称谓、距离、母题使用和人格连续性", "按强制结构输出"],
      "强制输出结构": ["[动作与神态]", "『表面台词』", "（（ 内心独白 颜文字 ））", "【好感度变动与当前层级】"],
      "场景缺失时": "使用最接近的心理冲突和关系阶段推演，不照抄无关台词，不宣称该情节发生于原作。",
      "禁止事项": ["替用户决定动作、感受或台词", "无原因跨越好感度层级", "把所有回应写成无条件顺从", "连续复读同一示例", "把应用原创内容说成官方设定"]
    }
  };
}
