...
5. 根据角色经历、母题和语料统计，用固定规则生成 -100至100 的双向好感度、12 类场景、5 档正面反应、4 档负面反应、动作、内心独白和 324 条新对白。
6. 使用 Galgame 式路线状态：选择后更新好感度、事件旗标、路线锁、角色记忆、修复窗口、专属章节和事件 CG 文本脚本。角色会主动疏离、隐瞒、欺骗、设局、反击、逃脱或逆转，不会只等用户发令。
7. 好感度低于 0 开启 4 个坏结局（含 1 个隐藏）；10/30/50/80 解锁角色剧情；100 点从 3 个普通和 5 个隐藏好结局中判定最匹配的一个。
8. 支持虚构角色，也支持为本人或已同意的现实人物填写简单/复杂人设。真人卡不生成露骨内容，应用推演的内心和剧情不会冒充本人真实想法。
9. 可选择关闭亲密、浪漫不露骨、纯爱成人、黄毛/NTR 成人或预先协商的黑暗权力幻想。露骨模式只适用于卡内相关人物均为 18 岁以上的虚构成年人，并要求当前场景明确、自愿、可撤回的同意。
10. 成人路线包含独立的 0–100 堕落值和性同意状态机；50 点开启确认窗口，但不会用数值替角色自动同意。
11. 导出 Character Card V3 JSON、Word DOCX 和启动指令 TXT。
...
1. 从 [Releases](https://github.com/huanglinfei091-cmd/canon-character-card/releases) 下载最新的 `CanonCharacterCard-Windows-x64-v1.2.0.zip`。
...
1. 从 [Releases](https://github.com/huanglinfei091-cmd/canon-character-card/releases) 下载最新的 `CanonCharacterCard-Android-v1.2.0.apk`。
